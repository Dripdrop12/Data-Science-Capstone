...
load_data <- function(){
  unigram_dt<<-readRDS(file.path('Data', 'unigram_hash.rds'))
  bigram_dt<<-readRDS(file.path('Data', 'bigram_hash.rds'))
  trigram_dt<<-readRDS(file.path('Data', 'trigram_hash.rds'))
  tetragram_dt<<-readRDS(file.path('Data', 'tetragram_hash.rds'))
}
...
get_words_full <- function(input_str){
  ...
  merged_long = merged[order(-sum_prob)][1:200]
  merged_short = merged[order(-sum_prob)][1:10]# Show top 10
