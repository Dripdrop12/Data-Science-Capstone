# Required libraries and source files #
library(NLP)
library(slam)
library(tm)
library(data.table)
...
shinyServer(
  function(input, output, session) {
    ...
    progress$set(message = 'Loading web-scraped text',
    ...
    dvalues <- reactive({
      ...
      for (e in names(input)){                
        ...
        if (input[[e]][1] == 1){
          ...
          the_sentence <- paste(input_str, new_word, sep = ' ')
          updateTextInput(session, "wt_1", value = the_sentence)
          break
        }
        a = a+1
      }
    ...
    output$tetracnts <- renderText({
      dvalues()
      return(paste0('Four Word Sequences: ', len_terta))
    })
    
    output$tricnts <- renderText({
      dvalues()
      return(paste0('Three Word Sequences: ', len_tri))
    })
    
    output$bicnts <- renderText({
      dvalues()
      return(paste0('Two Word Sequences: ', len_bi))
    })
    
    output$unicnts <- renderText({
      dvalues()
      return(paste0('Single Words: ', len_uni))
    })
})